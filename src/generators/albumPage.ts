...
export function parseInput({ formData, tracklistData, officialStreamingData, extLinksData }: RawInput): ProcessedInput {
  let {
  ...
  const officialStreamingLinks: { paramKey: string, paramValue: string }[] = [];
  for (let [name, url] of officialStreamingData) {
    name = (name || '').trim();
    url = (url || '').trim();
    if (name === '' || url === '') continue;
    const l = CONST_ALBUM_STREAMING_LINKS.filter(el => el.name === name);
    if (l.length === 0) continue;
    const paramKey = l[0].paramKey;
    const regex = l[0].regex;
    const embedId = (regex.exec(url)?.groups || {})['embedid'] || '';
    officialStreamingLinks.push({ paramKey, paramValue: embedId });
  }
  const extLinks: ExternalLink[] = extLinksData
    .map(arr => {
      let isAlbumReadMoreLink = false;
      const m = CONST_RECOGNIZED_LINKS.find(({ re }) => {
        return arr[0].match(re) !== null;
      });
      if (m?.isAlbumReadMoreLink === true) isAlbumReadMoreLink = true;
      return new ExternalLink(arr[0], arr[1], arr[2], undefined, undefined, isAlbumReadMoreLink);
    })
    .filter(el => el.url !== '');
  ...
  return {
    ...
    tracklist, officialStreamingLinks, extLinks
  }
}
...
export function validate(input: ProcessedInput): {
  ...
  let {
    ...
    tracklist, officialStreamingLinks
  } = input;
  ...
  if (officialStreamingLinks.length === 0) {
    ...
    if (invalidIds.length > 0) {
      const messages: IDictionary<string> = {
        'nn-xfade': 'Invalid embed URL for Niconico Crossfade: The link should be in the format of https://www.nicovideo.jp/watch/smXXXXXX',
        'yt-xfade': 'Invalid embed URL for YouTube Crossfade: The link should be in the format of https://www.youtube.com/watch?v=XXXXXXXXXXX where the YouTube video watch id is precisely 11 characters long',
        'sp-embed': 'Invalid embed URL for Spotify: The link should be in the format of https://open.spotify.com/album/XXXXXXXX',
        'yt-playlist': 'Invalid embed URL for YouTube Music: The link should be in the format of https://www.youtube.com/playlist?list=XXXXXXXXXXXXXXXXXXX',
        'bc-embed': 'Invalid embed ID for Bandcamp: The embed ID is purely numeric. Extract the Bandcamp embed ID from the iframe source code generated by Bandcamp\'s Share/Embed feature.'
      }
      for (let invalidId of invalidIds) {
        res.push([
          true,
          messages[invalidId.paramKey] || '',
          'official-streaming'
        ]);
      }
    }
