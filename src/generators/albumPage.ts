...
export function parseInput({ formData, tracklistData, officialStreamingData, extLinksData }: RawInput): ProcessedInput {
  ...
  for (let [name, url] of officialStreamingData) {
    ...
    const regex = l.regex;
    const embedId = ((regex.exec(url)?.groups || {}) as {'embedid': string})['embedid'] || '';
  ...
  const extLinks: ExternalLink[] = extLinksData
    .map(([url, description, isOfficial]) => {
      ...
      return new ExternalLink(
        url, description, isOfficial, 
        undefined, undefined, 
        m?.mapToAlbumInfoboxReadMoreParam
      );
    })
    .filter(el => el.url !== '');
  ...
  if (officialStreamingLinks.length === 0) {
    ...
    if (invalidIds.length > 0) {
      const messages: IDictionary<string> = {
        ...
        'sc-xfade': 'Invalid embed URL for SoundCloud: The link should be in the format of https://soundcloud.com/XXXXXXXXXX',
      }
